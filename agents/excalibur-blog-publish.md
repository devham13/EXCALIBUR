...
1. **Preflight:** link-verify с `--site-base $EXCALIBUR_PUBLIC_SITE_URL` (или `EXCALIBUR_PUBLIC_SITE_URL` из env).
...
9. **Live check:** `GET $EXCALIBUR_PUBLIC_SITE_URL/{slug}/` → HTTP 200. Иначе FAIL.
...
permalink=$EXCALIBUR_PUBLIC_SITE_URL/{slug}/
```
