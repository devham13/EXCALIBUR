...
| GEO / citability | 25 | 24 | Lead answer-first, таблица SEO vs GEO, 7 шагов ol, 7 FAQ, атомарные H2 |
...
| Contract HTML | 10 | 7 | linter PASS после fix H2; объём 8667 ✓, CTA ≤3 ✓; −3 нет `<img>` с alt (cover на ④a) |
...
| O04 | ✓ | ol (7 шагов), ul (чеклист 14 пунктов), table |
...
| Ept02 | ✓ | Все 6 ссылок HTTP 200 (link-verify pass) |
...
- total: 6, failed: 0
- OK: wordstat.yandex.ru, webmaster.yandex.ru, direct.yandex.ru, kv-ai.ru (×2), internal `/` (×2)
...
- verdict: PASS
- warn: water phrase «в этой статье вы узнаете» — в negation-контексте («Без …»), не blocker
- see `utility-gate-report.json`

## Fix cycle

- cycle 1: переименован H2 «Упакуйте материал для GEO: FAQ, schema, llms.txt» → «…schema, llms.txt и разметка» (html-linter duplicate FAQ)
- cycle 2: повтор GEO QA — PASS

## Optional (не blocker)

- добавить `<img>` с alt по контракту (cover на шаге ④a)
- author_id в meta: artur-horoshev (schema-агент)
