...
Indexer (2026-08-16): 0 auto-inject opportunities среди 5 статей в `memory/blog/articles` (keywords не совпали в чужих HTML без existing links). `llms.txt` / `llms-full.txt` обновлены в `memory/blog/`. После publish — проверить internal links; при новых статьях — перезапустить interlinker `--apply`.
