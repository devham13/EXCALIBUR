...
def merge_process_env(env: dict[str, str]) -> dict[str, str]:
    import os
    ...
    for key in keys:
        ...
        if value:
            merged[key] = value
    if not merged.get("FTP_PASS") and merged.get("FTP_PASSWORD"):
        merged["FTP_PASS"] = merged["FTP_PASSWORD"]
    if not merged.get("SFTP_PASSWORD") and merged.get("SSH_PASSWORD"):
        merged["SFTP_PASSWORD"] = merged["SSH_PASSWORD"]
    if not merged.get("SFTP_HOST") and merged.get("SSH_HOST"):
        merged["SFTP_HOST"] = merged["SSH_HOST"]
    if not merged.get("SFTP_USER") and merged.get("SSH_USER"):
        merged["SFTP_USER"] = merged["SSH_USER"]
    if not merged.get("PUBLIC_SITE_URL") and merged.get("EXCALIBUR_PUBLIC_SITE_URL"):
        merged["PUBLIC_SITE_URL"] = merged["EXCALIBUR_PUBLIC_SITE_URL"]
    return merged


def trigger_bootstrap_url(url: str) -> str:
    out = ""
    try:
...
def publish_via_sftp(env: dict[str, str], php: str, public_base: str) -> str:
    ...
    host = env.get("SFTP_HOST") or env.get("SSH_HOST") or ""
    user = env.get("SFTP_USER") or env.get("SSH_USER") or ""
    passwd = env.get("SFTP_PASSWORD") or env.get("SSH_PASSWORD") or ""
    port = int(env.get("SFTP_PORT") or env.get("SSH_PORT") or "22")
    if not host or not user or not passwd:
        raise RuntimeError("SFTP credentials missing (SFTP_HOST/SFTP_USER/SFTP_PASSWORD or SSH_*)")

    transport = paramiko.Transport((host, port))
    transport.connect(username=user, password=passwd)
    sftp = paramiko.SFTPClient.from_transport(transport)
    print(f"SFTP upload to {host}:{port} ...")
...
def publish_via_ftp(env: dict[str, str], php: str, public_base: str) -> str:
    ...
    try:
        ...
        if "425" in str(exc) or "Bad IP" in str(exc):
            print(f"FTP STOR blocked ({exc}); falling back to SFTP...")
...
def main() -> int:
    ...
    env = merge_process_env(load_env(root))
