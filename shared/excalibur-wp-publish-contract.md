...
- `memory/site.env.local` — FTP + `FTP_ROOT` (корень WP, где `wp-load.php`) + `EXCALIBUR_PUBLIC_SITE_URL=<production URL>` + `EXCALIBUR_BLOG_ALLOW_PUBLISH=yes`
...
Тема WordPress на **production host** может **дописывать** после контента второй блок «Часто задаваемые вопросы по теме (FAQ)» с универсальными вопросами — это **не** часть `article.html`.
